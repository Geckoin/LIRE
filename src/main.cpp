...
uint256 hashGenesisBlock("0x00000b5d27ad9f15b31bbb182d5677259a09ed8a412c93d36e755a56445ce5f4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LIRE: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LIRE Signed Message:\n";
...
{
    // LIRE: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
	...
	if (IsCoinBase())
		{
			if (vin[0].scriptSig.size() < 2 || vin[0].scriptSig.size() > 100) // <2 > 100
				return state.DoS(100, error("CTransaction::CheckTransaction() : coinbase script size value=%d", vin[0].scriptSig.size()));
		}
		else
		{
			BOOST_FOREACH(const CTxIn& txin, vin)
...
static const int64 nTargetTimespan = 24 * 60 * 60; // LIRE: 1 day
static const int64 nTargetSpacing = 2.5 * 60; // LIRE: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, LIRE - DarkGravity v2, written by Evan Duffield - evan@darkcoin.io */
...
{
    // LIRE: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000b5d27ad9f15b31bbb182d5677259a09ed8a412c93d36e755a56445ce5f4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		...
		const char* pszTimestamp = "Dalla repubblica 23/Apr/2014 Renzi-Enrico Letta";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
		block.nTime = 1398240200;
        block.nBits    = 0x1e0ffff0;
		block.nNonce = 5244704;
        ...
        assert(block.hashMerkleRoot == uint256("0x405b1ad67208262a0b8e6a52694de5d92f11569297245aa9b28acd7a93c86ece"));
...
unsigned char pchMessageStart[4] = { 0xd3, 0xed, 0xc9, 0xf1 }; // LIRE
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LIREMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LIREMiner : ProcessBlock, block not accepted");
    }
